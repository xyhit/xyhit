<?php
	...
	if (isset($_GET['userid'])){
		$userid = $_GET['userid'];
		include ('./conn.php');
		$sql = mysql_query("select userid from mark where markerid='$userid' order by time desc", $db);
	}
?>
<html>
<head>
			...
			if ($sql){
				...
				if ($num <= 0){
					...
					while(($result = mysql_fetch_array($sql)) && $counter < 10){
						$markerquery = mysql_query("select name, company, position from user where id='".$result['userid']."'", $db);
						if ($markerquery){
							$markerresult = mysql_fetch_array($markerquery);
							$imgsrc = "http://xyhit-headimg.stor.sinaapp.com/".$result['userid'].".png";
							$userinfourl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx47b09b3c8dd8305e&redirect_uri=".urlencode("http://1.xyhit.applinzi.com/userinfo.php?id=".$result['userid'])."&response_type=code&scope=snsapi_base&state=124#wechat_redirect";
